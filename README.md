...
> [!WARNING]
> **关于旧版本备份的安全说明：**
> 如果你是极其早期的旧版用户，之前的版本为了方便可能使用过固定的遗留硬编码密钥（例如 `aether-ssh-secret-key-0123456789`）。这种方式容易导致如果别人的软件拿到你的配置文件，就能直接解密。
> **解决办法**：新版 AetherSSH 已经内建了自动迁移机制。只要你使用新版本打开过一次旧的配置，后台就会瞬间静默、无感地用全新随机生成的本地专属密钥对数据进行重新加密覆盖，彻底告别硬编码风险！强烈建议所有老用户尽快下载新版覆盖运行一次。
